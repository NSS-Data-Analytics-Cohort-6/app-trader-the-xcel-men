...
-- TAKE TWOOOOOOOOOOOOOOOOOOOOO
SELECT
	main.name AS app,
	main.avg_price,
	((main.longevity_in_years * 48000) - main.purchase_price) AS total_profit,
	main.avg_rating,
	main.longevity_in_years,
	main.content_rating,
	main.primary_genre,
...
FROM
		...
		(CASE
			WHEN ROUND((a.rating + s.avg_rating)/2,1) >= 0 AND ROUND((a.rating + s.avg_rating)/2,1) <= .2 THEN 1
	 		...
	 		ELSE 11 END) AS longevity_in_years,
	 												-- ^ Rating Nearest 5
		(CASE
			WHEN (a.price + CAST(REPLACE(s.price,'$','') AS float)/2) = 0 THEN 10000 
	 		WHEN (a.price + CAST(REPLACE(s.price,'$','') AS float)/2) > 0 AND (a.price + CAST(REPLACE(s.price,'$','') AS float)/2) <= 1 THEN 10000
			ELSE ((a.price + CAST(REPLACE(s.price,'$','') AS float)/2) * 10000) END) as purchase_price
	FROM app_store_apps AS a 
	INNER JOIN 
		(SELECT name, 
	 	price, 
	...
	ON a.name = s.name) AS main          
GROUP BY name, avg_price, longevity_in_years, purchase_price, avg_rating, content_rating, primary_genre
ORDER BY total_profit DESC, avg_rating DESC