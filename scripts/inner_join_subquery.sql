...
WHERE ROUND((a.rating + p.rating)/2,2) > 4.5 
	AND CAST(REPLACE(REPLACE(p.install_count,'+',''),',','') as float) > 10000000
ORDER BY (CAST(a.review_count as int) + p.review_count)/2 DESC, CAST(REPLACE(REPLACE(p.install_count,'+',''),',','') as float) DESC
